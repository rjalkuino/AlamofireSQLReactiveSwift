...
extension SignalProtocol {
    ...
    public func transformToBool() -> Signal<Bool, Never> {
        return Signal { observer,sink in
            signal.observe { event in
                switch event {
    ...
    public func failedAsNext(fn: @escaping () -> Value) -> Signal<Value, Never> {
        return Signal { observer,sink in
            signal.observe { event in
                switch event {
...
extension SignalProducerProtocol {
    ...
    public func transformToBool() -> SignalProducer<Bool, Never> {
        return producer.lift { $0.transformToBool() }
    }
    
    public func failedAsNext(fn: @escaping () -> Value) -> SignalProducer<Value, Never> {
